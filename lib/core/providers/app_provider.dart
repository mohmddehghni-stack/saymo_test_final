...
class AppProvider extends ChangeNotifier {
  ...
  String? get coupleId => _coupleId; // 🔥 جدید
  ...
  void setUserId(String id) async {
    _userId = id;
    ApiService.currentUserId = id; // همگام‌سازی با ApiService
    notifyListeners();
    final prefs = await SharedPreferences.getInstance();
  ...
  void connectPartner(
    ...
    if (partnerId != null) await prefs.setString('partnerId', partnerId);
    if (displayName != null)
  ...
  // ──── خروج و حذف حساب (با پاک کردن توکن) ────

  void logout() async {
    await ApiService.clearToken(); // 🔥 پاک کردن توکن از RAM و Hive
  ...
  Future<void> deleteAccount() async {
    try {
      ...
      if (response.statusCode == 200) {
        await ApiService.clearToken(); // 🔥 پاک کردن توکن
  ...
  Future<void> loadFromStorage() async {
    ...
    // همگام‌سازی با ApiService
    if (_userId != null) setUserId(_userId!);
    if (_coupleId != null) setCoupleId(_coupleId!);
